	...
	init: function( editor ) {
		editor.addCommand('simage', {
			exec: function (editor) {
				...
				a.setAttribute('accept', '.jpg,.jpeg,.png')
				a.click()
				a.onchange = function(){
					file = a.files[0];
					...
					$(CKEDITOR.instances[currentInstance]).trigger('enableFormSubmit')
					curr =  CKEDITOR.instances[currentInstance];
					if (file.size > 5000000){
						...
						c.innerHTML = 'X'
						b.appendChild(c)
						e = document.querySelector('#body_error')
						e.appendChild(b)
						setTimeout(function(){ 
						...
						return
					}else if (['jpeg','jpg','png'].indexOf(file.type.split('/')[1]) === -1){
						b = document.createElement('div')
						b.className = 'message alert alert-danger'
						m = document.createElement('span')
